...
public class ImagePlayer {
    ...
    public ImagePlayer(FrameBufferPoolWithQueue pool, Clock clock, int textureId, int width, int height) {
        this.pool = pool;
        this.clock = clock;
    ...
    /** 渲染线程调用：每渲染帧调用：按音频主时钟门控，到点才上屏，未到点等待（不弹出）。 */
    public void render() {
        ...
        if (!clock.switchImage(head.ptsUs)) return;
